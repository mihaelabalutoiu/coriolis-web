...
/**
 * Reads a `LicenceStats` body out of the licensing server response.
 *
 * Licensing servers which predate the SAP licence type report the standard
 * allowance as flat fields on the `appliance_licence_status` root instead of
 * under `standard_licence_stats`, so the root is used as the fallback body.
 */
