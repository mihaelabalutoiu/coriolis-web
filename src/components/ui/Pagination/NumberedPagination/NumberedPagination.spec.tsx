...
describe("NumberedPagination", () => {
  ...
  it("handles previous and next click", async () => {
    ...
    const previousButton = screen.getByRole(
  ...
  it("handles disabled states", async () => {
    ...
    let previousButton = screen.getByRole(
      "button", { name: "Previous" });
    let nextButton = screen.getByRole(
      "button", { name: "Next" });
    ...
    previousButton = screen.getByRole(
      "button", { name: "Previous" });
    nextButton = screen.getByRole(
      "button", { name: "Next" });
  ...
  it("renders all items per page options", () => {
    ...
    itemsPerPageOptions.forEach((option, index) => {
      expect((
        options[index] as HTMLOptionElement).value).toBe(option.toString());
