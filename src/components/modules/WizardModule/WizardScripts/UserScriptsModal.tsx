...
// One selected script per phase. `content` is null when empty so we never emit
// an empty-string payload. `fileName` is null for scripts loaded from the API
// (which only carry the payload).
...
class UserScriptsModal extends React.Component<Props, State> {
  ...
  handleSave() {
    ...
    >((acc, phase) => {
      ...
      // Skip empty phases entirely — never send an empty-string payload.
