...
describe("WizardScripts", () => {
  ...
  it("renders without crashing", () => {
    ...
    // Every row offers a "Choose Scripts" action when nothing is configured.
  ...
  it("opens the per-phase modal showing all three phases", () => {
    ...
    // The modal renders into a portal (document.body), so query via `screen`.
    expect(screen.getByText("Windows Script File - User Scripts")).toBeTruthy();
    expect(screen.getByText("OS morphing: before OS mount")).toBeTruthy();
    expect(screen.getByText("OS morphing: after OS mount")).toBeTruthy();
    expect(screen.getByText("Replica first boot")).toBeTruthy();
    // Each empty phase offers a file picker (no paste textarea).
